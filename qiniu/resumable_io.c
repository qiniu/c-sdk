...
static Qiniu_Error Qiniu_Rio_Mkblock(
    Qiniu_Client *self, Qiniu_Rio_BlkputRet *ret, int blkSize, const char *upHost,
    Qiniu_Reader body, int bodyLength, Qiniu_Rio_PutExtra *extra)
{
...
{
    const char *url = Qiniu_String_Format(1024, "%s/bput/%s/%d", ret->host, ret->ctx, (int)ret->offset);
    Qiniu_Error err = Qiniu_Rio_bput(self, ret, body, bodyLength, url);
    Qiniu_Free((void *)url);
    ...
    Qiniu_Rio_PutExtra *extra, size_t *pChunksUploaded)
{
    ...
    if (ret->ctx == NULL)
    {
        ...
        for (int tries = 0; tries < extra->tryTimes && tries <= c->retriesMax; tries++)
        {
            ...
            err = Qiniu_Rio_Mkblock(c, ret, blkSize, upHost, body, bodyLength, extra);
            if (err.code == Qiniu_OK.code)
            {
                break;
            }
    ...
    while ((int)(ret->offset) < blkSize)
    {
        ...
        if (tryTimes > c->retriesMax + 1)
        {
            tryTimes = c->retriesMax + 1;
        }
    ...
    const char *uptoken, const char *key, Qiniu_ReaderAt f, Qiniu_Int64 fsize, Qiniu_FileInfo *fi, Qiniu_Rio_PutExtra *extra1, Qiniu_Rio_Recorder *recorder)
{
    ...
    if (extra.upHost == NULL && (extra.upHosts == NULL || extra.upHostsCount == 0))
    {
        ...
        if (err.code != Qiniu_OK.code)
        {
            Qiniu_Multi_Free(2, (void *)accessKey, (void *)bucketName);
            return err;
        }
    }
...
reinit:
    ...
    else
    {
        ...
        for (int tries = 0; tries < extra.tryTimes && tries <= self->retriesMax; tries++)
        {
            const char *upHost = upHosts[tries % upHostsCount];
            ...
            else if (err.code == Qiniu_OK.code || _Qiniu_Should_Retry(err.code) == QINIU_DONT_RETRY)
            {
